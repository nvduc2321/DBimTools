...
{
    public class MainHole1Wall2 : MainHole1Element
    {
        public MainHole1Wall2(RevHole1 revHole1Info) : base(revHole1Info)
        {
            VtX = GetVtX();
            VtY = GetVtY();
            VtZ = GetVtZ();
        ...
        public override RevOpening GetOpening()
        {
            ...
            try
            {
                if (RevHole1Info.HoleCircle2 && RevHole1Info.HoleRect2)
                    return null;
                if (RevHole1Info.HoleCircle2)
                {
                    var opening = new RevOpeningCircle();
                    opening.VTX = VtX;
                    opening.VTY = VtY;
                    opening.VTZ = VtZ;
                    opening.Radius = RevHole1Info.LoMo2_R;
                    opening.Center = Origin - VtZ * (RevHole1Info.ChieuCaoBung / 2 - RevHole1Info.LoMo2_dV - RevHole1Info.LoMo2_R);
                    opening.Lines = opening.GetLines();
                    result = opening;
                }
                if (RevHole1Info.HoleRect2)
                {
                    var opening = new RevOpeningRect();
                    opening.VTX = VtX;
                    opening.VTY = VtY;
                    opening.VTZ = VtZ;
                    opening.Width = RevHole1Info.LoMo2_B;
                    opening.Height = RevHole1Info.LoMo2_H;
                    opening.Center = Origin - VtZ * (RevHole1Info.ChieuCaoBung / 2 - RevHole1Info.LoMo2_dV - RevHole1Info.LoMo2_H / 2);
                    opening.Lines = opening.GetLines();
                    result = opening;
                }
            }
        ...
        public override XYZ GetOrigin()
        {
            return RevHole1Info.Origin
                ...
                - RevHole1Info.VTX * (RevHole1Info.ChieuRongGa / 2 - RevHole1Info.ChieuDayTuong / 2);
        }
