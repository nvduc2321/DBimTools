...
{
    public class MainHole1Wall4 : MainHole1Element
    {
        public MainHole1Wall4(RevHole1 revHole1Info) : base(revHole1Info)
        {
            VtX = GetVtX();
            VtY = GetVtY();
            VtZ = GetVtZ();
        ...
        public override RevOpening GetOpening()
        {
            ...
            try
            {
                if (RevHole1Info.HoleCircle4 && RevHole1Info.HoleRect4)
                    return null;
                if (RevHole1Info.HoleCircle4)
                {
                    var opening = new RevOpeningCircle();
                    opening.Normal = VtY;
                    opening.Radius = RevHole1Info.LoMo4_R;
                    opening.Center = Origin - VtZ * (RevHole1Info.ChieuCaoBung / 2 - RevHole1Info.LoMo4_dV - RevHole1Info.LoMo4_R);
                    result = opening;
                }
                if (RevHole1Info.HoleRect4)
                {
                    var opening = new RevOpeningRect();
                    opening.Normal = VtY;
                    opening.Width = RevHole1Info.LoMo4_B;
                    opening.Height = RevHole1Info.LoMo4_H;
                    opening.Center = Origin - VtZ * (RevHole1Info.ChieuCaoBung / 2 - RevHole1Info.LoMo4_dV - RevHole1Info.LoMo4_H / 2);
                    result = opening;
                }
            }
            catch (Exception)
